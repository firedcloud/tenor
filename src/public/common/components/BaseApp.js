...
export class BaseApp extends CustomComponent {
    ...
    render() {
        ...
        this.context.featureFlags = {
            ...
            staticImageBetaEnabled: process.env.BROWSER && this.context.arianeMultivariateGroupSelect({
                1: true,
                2: true,
                3: true,
                4: true,
        ...
        this.context.countryCode = process.env.BROWSER && cookie.parse((document && document.cookie) || '').countryCode;
        ...
        this.context.authService && this.context.authService.checkGoogleAccessTokenExpiration && this.context.authService.checkGoogleAccessTokenExpiration(this.context.apiService);

        this.context.store = appGlobals.store;

        const children = this.props.children;
        const s = children ? < div id = "view" > {
            children
        } < /div> : <div id="view"/ > ;
