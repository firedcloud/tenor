...
import { CustomComponent, Button } from "../components";
...
export class GoogleSigninButton extends CustomComponent {
  ...
  handleGoogleAuthLoadChange() {
    ...
    if (googleAuthService.errorMsg) {
      this.props.failureCallback &&
        this.props.failureCallback(
          googleAuthService.errorMsg,
          "Google authentication failed to initialize"
        );
  ...
  signInWithGoogle() {
    ...
    GoogleAuth.signIn(googleAuthService.getSignInOptions()).then(
  ...
  handleAuthError(error) {
    ...
    console.error(error);
    this.props.failureCallback &&
      this.props.failureCallback(error, "Google authentication error");
  }

  render() {
    return (
      <div className="GoogleSigninButton">
        <label className="hidden" for="google-auth-button">
          {" "}
          Sign in with Google{" "}
        </label>{" "}
        <Button
