...
import {
    AuthDialog
} from '../common/dialogs/AuthDialog';
import {
    BaseApp
} from '../common/components/BaseApp';
import {
    HTTP404Page
} from '../common/components/http404';
import {
    CONSTANTS,
    initConfig
} from '../common/config';
...
import {
    BrandedPartnerPage
} from './pages/profilePartner';
import {
    GifMakerPage
} from './pages/gif-maker';
import {
    EmotionalGraphPage
} from './pages/about/emotional-graph';
import {
    PartnerSpotlightPage
} from './pages/about/partner-spotlight';
import {
    PressPage
} from './pages/about/press';
import {
    AboutFAQPageRedirect
} from './pages/about/about-faq';
import {
    AboutUsPage
} from './pages/about/about-us';
import {
    ContactPage
} from './pages/about/contact';
import {
    LegalTermsPage
} from './pages/about/legal-terms';
import {
    LegalTermsUpdateRedirect
} from './pages/about/legal-terms-update';
import {
    LegalTermsDeveloperUpdateRedirect
} from './pages/about/legal-terms-developer-update';
import {
    LegalPrivacyPage
} from './pages/about/legal-privacy';
import {
    LegalPrivacyUpdateRedirect
} from './pages/about/legal-privacy-update';
import {
    LGLegalDataUsagePage,
    LGLegalPrivacyRedirect
} from './pages/about/lg';
import {
    ForgotPasswordPage
} from './pages/about/forgot-password';
import {
    IMessagePage
} from './pages/about/imessage';
import {
    MacPage
} from './pages/about/mac';
...
import {
    OptInDialog
} from './components/OptInDialog';
import {
    FlagDialog
} from './components/FlagDialog';
import {
    EditProfileDialog
} from './components/EditProfileDialog';
import {
    ConfirmationDialog
} from '../common/dialogs/ConfirmationDialog';
import {
    CaptionShareDialog
} from './components/CaptionShareDialog';
import {
    StaticImageDialog
} from './components/dialogs/StaticImageDialog';
import {
    PopupDialog
} from '../common/dialogs/PopupDialog';
...
class App extends BaseApp {
    beforeChildren() {
        const currentPath = this.context.gtService.delocalizeUrlPath(this.context.router.history.location.pathname);
        const showNav = [
            '/mac',
            '/imessage',
        ...
        return [
            showNav && < NavBar / > ,
            showSearch && < TopSearchBar / > ,
        ];
    }
    afterChildren() {
        return ( < DialogContainer dialogMap = {
                {
                    'auth-dialog': {
                        'class': AuthDialog,
                        'options': {
    ...
    function routesForLocale(appGlobals) {
        ...
        const pathMap = {
            ...
            'emotional-graph': gtService.gt.pgettext('url path component', 'emotional-graph'),
            'partners': gtService.gt.pgettext('url path component', 'partners'),
            'contentpartners': gtService.gt.pgettext('url path component', 'contentpartners'),
            ...
            'legal-terms-update': gtService.gt.pgettext('url path component', 'legal-terms-update'),
            'legal-terms-developer-update': gtService.gt.pgettext('url path component', 'legal-terms-developer-update'),
            'legal-privacy': gtService.gt.pgettext('url path component', 'legal-privacy'),
            'legal-privacy-update': gtService.gt.pgettext('url path component', 'legal-privacy-update'),
            'lg-legal-privacy': gtService.gt.pgettext('url path component', 'legal/privacy'),
            'lg-legal-data-usage': gtService.gt.pgettext('url path component', 'legal/data-usage'),
            'forgot-password': gtService.gt.pgettext('url path component', 'forgot-password'),
        ...
        /> <
        Route path = {
            `${path}${pathMap['search']}/:searchData`
        }
        component = {
            SearchPage
        }
        ...
        /> <
        Route path = {
            `${path}${pathMap['explore']}/:searchData`
        }
        component = {
            ExplorePage
        }
        ...
        /> <
        Route path = {
            `${path}${pathMap['users']}/:username/:mediatype?`
        }
        component = {
            UserProfilePage
        }
        /> <
        Route path = {
            `${path}${pathMap['official']}/:username/:mediatype?`
        }
        component = {
            BrandedPartnerPage
        }
        ...
        /> <
        Route path = {
            `${path}${pathMap['legal-terms-update']}`
        }
        component = {
            LegalTermsUpdateRedirect
        }
        /> <
        Route path = {
            `${path}${pathMap['legal-terms-developer-update']}`
        }
        component = {
            LegalTermsDeveloperUpdateRedirect
        }
        ...
        /> <
        Route path = {
            `${path}${pathMap['legal-privacy-update']}`
        }
        component = {
            LegalPrivacyUpdateRedirect
        }
        ...
        /> <
        Route path = {
            `${path}${pathMap['lg-legal-data-usage']}`
        }
        component = {
            LGLegalDataUsagePage
        }
    ...
    export function routes(history, config, request, response, context) {
        ...
        if (process.env.BROWSER) {
            ...
            window.addEventListener('load', function() {
                ...
                } else if (effectiveType && (effectiveType.endsWith('2g') || effectiveType.endsWith('3g'))) {
                    dataSaverMode = true;
