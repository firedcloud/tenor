...
import { subscribe } from "../../../../replete";
import { TaggingView } from "./views/TaggingView";
import { UploadView } from "./views/UploadView";
import { ImageViewer } from "./views/ImageViewer";
import { Metadata } from "../../../common/metadata";

import { ITEM_STATES, SETTINGS } from "./constants";
import { createMediaFormatsList } from "./util";
import { getFileExtension, isValidFileType } from "../../../common/util/files";
...
export class GifMakerPage extends Page {
  ...
  pageInit(props) {
    ...
    this.description = gettextSub(
      "Upload animated GIFs to Tenor to share on social media channels, text messages, and everywhere else."
    );
  }
  ...
  defaultState() {
    ...
    this.context.store.set(
      "ui.GifMakerPage.view",
      this.props.itemPageCaptioning.status ? "editing" : "upload"
    );
    return defaultState;
  }
  ...
  addItemsToQueue(data) {
    ...
    for (
      let i = 0;
      i < data.length && i + this.state.queue.length < maxUploads;
      i++
    ) {
      const datum = data[i];
      const datatype = typeof datum === "string" ? "url" : "file";
      console.log(
        `FILE PICKER -- FILE_${i}, name: ${datum.name}, size: ${datum.size}, type: ${datum.type}`
      );
      ...
      if (
        ...
        this.queuePromises.push(
          this.addToQueuePromise({
            [datatype]: datum,
          })
        );
      } else {
        this.updatePageErrors({
          type: "filetype",
          message: `Failed to load. Improper file type: ${getFileExtension({
            [datatype]: datum,
          })}.`,
        });
      }
    ...
    Promise.all(this.queuePromises).then(() => {
      ...
      if (!queue.length) {
        ...
        !this.props.itemPageCaptioning.status &&
  ...
  removeGifFromQueue(i) {
    const queue = this.state.queue
      .slice(0, i)
      .concat(this.state.queue.slice(i + 1));
    const finishedUploading = queue.every(
      (uploadObject) => uploadObject.uploadStatus === ITEM_STATES.DONE
    );
  ...
  parsePageErrors(errorQueue) {
    ...
    if (uploadErrors) {
      pageErrorMessages.push(
        `There was an issue uploading ${uploadErrors} of your files. Please edit your upload${
          uploadErrors > 1 ? "s" : ""
        } and try again.`
      );
    }
    ...
    if (processingErrors) {
      pageErrorMessages.push(
        `There was an issue processing ${processingErrors} of your files.`
      );
    }
    if (filetypeErrors) {
      ...
      pageErrorMessages.push(
        `${filetypeErrors} of your uploads could not be processed since ${msg}.`
      );
    }
    if (otherErrors) {
  ...
  trackEvent({ eventName, params = {}, gaParams = {} }) {
    ...
    if (
      !gaParams.hasOwnProperty("eventValue") &&
      params.hasOwnProperty("viewindex")
    ) {
      ...
      <div className="GifMakerPage">
        ...
        {this.props.view === "upload" && (
          <UploadView
            ...
            pageErrorsUpdated={this.state.pageErrorsUpdated}
          />
        )}{" "}
        ...
        {(this.props.view === "imageViewer" ||
          ...
          <ImageViewer queue={this.state.queue} />
        )}{" "}
      </div>
    );
  }
