...
export class BaseProfilePage extends Page {
    pageChanged(nextProps) {
        return nextProps.username !== this.props.username || nextProps.section !== this.props.section;
    }
    ...
    handleProfileBody(props) {
        ...
        if (!props.profile.pending) {
            ...
            this.description = gettextSub('Check out {username}\'s GIFs on Tenor. Discover, search and share popular GIFs with friends on Tenor.', {
                username: this.user.username
            });
        }
    }
    ...
    getMoreGifs() {
        this.context.store.call(
            'api.gifs.searchByUsername.*', [this.props.username, this.props.section, this.props.ownsProfile],
            'more'
        );
    }
    @autobind
    getMoreStickers() {
        this.context.store.call(
            'api.stickers.searchByUsername.*', [this.props.username, this.props.section, this.props.ownsProfile],
            'more'
        );
    }
    ...
    @autobind
    setSection(section) {
        this.context.store.set('ui.ProfilePage.*.section', [this.props.username], section);
    }
    ...
    renderStickerListSection() {
        ...
        const isPartner = usertype === 'partner';
        const header = `${isPartner ? partnername : username} ${gettextSub('Stickers')}`;
        ...
        return ( <
            ...
            h2 > {
                ...
                to = {
                        `/${isPartner ? 'official' : 'users'}/${this.props.username}`
                    } > {
                        gettextSub('See GIFs')
                    } <
                    /Link>
                ...
                to = {
                        `/${isPartner ? 'official' : 'users'}/${this.props.username}/stickers`
                    } > {
                        gettextSub('See all')
                    } <
                    /Link>
            ...
            /div> {
                ...
                this.stickerList.length > 0 && this.profilePageView && this.props.isMobile &&
                    <
                    GifList
                className = 'stickers'
                gifs = {
                    this.stickerList.slice(0, 6)
                }
                loaded = {
                    this.props.stickers.loaded
                }
