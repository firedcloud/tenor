...
import { Link, Page } from "../../common/components";
...
import { RelatedTag } from "../components/Tag";
...
@transformProps((props, context) => {
  ...
  props.searchData = searchData
    ? cleanQuery(decodeURIComponent(searchData))
    : undefined;
  props.searchMediaType = searchData
    ? context.gtService.parseURLSearchMediaType(searchData)
    : PAGE_MEDIA_TYPE.GIFS;
  props.searchTerms = searchData
    ? context.gtService.parseURLSearchTags(props.searchData)
    : [];
...
export class SearchPage extends Page {
  ...
  pageInit(props) {
    ...
    this.context.response.surrogateKey(
      "SearchPage",
      "/search/",
      `term:${encodeURIComponent(props.searchTermsDashed)}`
    );
  ...
  processNewProps(props) {
    if (!props.searchData) {
      ...
      this.noindex =
        ...
        !props.exploreData.results.length;
    }
  }
  ...
  getMoreSearchResults() {
    this.context.store.call(
      "api.gifs.search",
      [this.props.searchTermsSpaced, this.props.contentfilter],
      "more"
    );
  }
  @autobind
  ...
  @autobind
    ...
    this.setCanonicalURL(
      this.linkToSearch(props.searchTerms, props.searchMediaType)
    );
  ...
  handleExploretermBody(props) {
    ...
    if (
      body.results &&
      body.results.length &&
      localURL(body.results[0].url).substr(0, 8) === "/search/"
    ) {
  ...
  mergeTermLists() {
    ...
    if (exploreData && exploreData.keyword_searches.length) {
      ...
      this.props.searchSuggestions.results &&
      this.props.searchSuggestions.results.length > 0
    ) {
      ...
      selectItem(
        [
          () => {
            ...
            this.title = gettextSub(
              "{titleFragment} GIFs | Tenor",
              titleFragmentObj
            );
            this.description = gettextSub(
              "With Tenor, maker of GIF Keyboard, add popular {titleFragment} animated GIFs to your conversations. Share the best GIFs now >>>",
              titleFragmentObj
            );
          },
          () => {
  ...
  renderPage() {
    ...
    const stickerSearchPage =
      this.props.searchMediaType === PAGE_MEDIA_TYPE.STICKERS;

    return (
      <div className="SearchPage container page">
        <Metadata page={this}>
          <meta
            name="twitter:app:url:googleplay"
            content={`riffsy://search/${spaceSeparatedSearchTerms}`}
          />
          <meta
          ...
          <link
            rel="alternate"
            href={`android-app://com.riffsy.FBMGIFApp/riffsy/search/${spaceSeparatedSearchTerms}`}
          />
        ...
        >
          <meta itemprop="url" content={this.canonicalURL} />{" "}
          <meta itemprop="mainEntityOfPage" content={this.canonicalURL} />{" "}
          <meta itemprop="keywords" content={this.keywords} />{" "}
          <meta itemprop="headline" content={this.title} />{" "}
          <meta itemprop="name" content={this.title} />{" "}
          {this.h1_title.length && <h1> {this.h1_title} </h1>}{" "}
          {this.props.searchData && (
            <div className="search">
              {" "}
              {Object.keys(relatedTags).length > 0 && (
                <ul
                  className={`list-unstyled TagList ${
                    isMobile() ? "" : "no-wrap"
                  }`}
                  style={{
                    width: "auto",
                ...
                <div>
                  <div className="section-header-row">
                    ...
                    <Link
                      ...
                      to={`/search/${this.props.searchTerms.join(
                        "-"
                      )}-stickers`}
                      external={false}
                  ...
                  {this.props.isMobile ? (
                    <MobileCarousel
                      items={this.props.stickers.results}
              ...
              <div className="section-header-row">
                <h2> {stickerSearchPage ? "Stickers" : "GIFs"} </h2>
                {stickerSearchPage && (
                  <Link
                    className="switch-view-link"
                    to={`/search/${this.props.searchTerms.join("-")}-gifs`}
                    external={false}
              ...
              <GifList
                query={this.props.searchData}
                gifs={
                  stickerSearchPage
                    ? this.props.stickers.results
                    : this.props.gifs.results
                }
                loaded={
                  stickerSearchPage
                ...
                pending={
                  stickerSearchPage
                    ? this.props.stickers.pending
                    : this.props.gifs.pending
                }
        ...
        <UpsellPill
          text={gettextSub("Search from GIF Keyboard")}
